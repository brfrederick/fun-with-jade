...
var createDog = function(req, res) {
  ...
  // Save the new Dog obj to the database
...
// Update the age of a dog in the database if it exists
var updateDog = function(req, res){
  // Verify the user entered a name to search on
  if(!req.query.name){
    return res.json({err:'Name is required to search.'});
  }
  ...
  Dog.findByName(req.query.name, function(err,doc){
    ...
    // If no Dogs found
    if(!doc){
      return res.json({err: 'No Dogs found with supplied name'});
    }

    // Increment the dogs age
    doc.age++;
    ...
    // If doesn't exist, return 'No Dog with that name'
